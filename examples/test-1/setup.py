...
def setup(use_policy_layer=True):
    """Set up the agent with persistent wallet storage and optional policy layer.

    Args:
        use_policy_layer: Whether to enable the policy layer (default: True)
    ...
    agent_executor, wallet_provider = initialize_agent(
        config,
    ...
    agent_config = {"configurable": {"thread_id": "AgentKit Chatbot"}}
